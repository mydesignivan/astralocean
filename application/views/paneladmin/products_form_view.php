...
<form id="form1" method="post" action="<?=site_url('/panel/products/'. (!isset($info) ? "create" : "edit"));?>" enctype="multipart/form-data">
    <fieldset>
        ...
        <div class="clear left">
            ...
            <?php }else{?>
                <a href="<?=UPLOAD_DIR.$info['image'];?>" class="jq-fancybox"><img src="<?=UPLOAD_DIR.$info['image_thumb'];?>" alt="" width="200" height="146" /></a><br />
                <input type="file" name="txtImage" id="txtImage" value="" class="left larger" size="30" />
            <?php }?>
...
<!--
    Products.initializer(<?=!isset($info) ? 'false' : 'true';?>, '<?=$this->session->flashdata('savestatus')?>');
-->
</script>
